...
export function buildSingboxJson(
  ...
  const config = {
    ...
    // Happ (and sing-box CLI) reject a config with no `inbounds` as "invalid
    // JSON", unlike Hiddify/NekoBox which inject their own tun. A single modern
    // tun inbound makes the config complete for every sing-box client (GUI
    // clients still override it with their own OS-managed tun) WITHOUT opening a
    // local SOCKS/HTTP proxy — no `mixed`/`socks` inbound, so no localhost leak.
    // This is what lets Happ show the named node list AND honour route rules
    // (ru-split) at once; the old no-inbounds body forced Happ onto the plain
    // (routing-less) format.
    inbounds: [
      {
        type: 'tun',
