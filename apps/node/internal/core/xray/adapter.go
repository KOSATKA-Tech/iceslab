...
// {password,email}], etc).
//
// listen+port are REQUIRED even though adu never binds the socket: xray's
// conf.InboundDetour validation rejects "Listen on AnyIP but no Port(s) set",
// so omitting the port made adu add 0 users (exit 0) and the caller fell back
// to a full restart on EVERY user add — dropping all live connections on the
// node. Mirror the full render (config.go) so the payload validates.
