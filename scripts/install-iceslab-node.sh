...
banner() {
  ...
  printf '\n'
  printf '  v0.1.8  ·  github.com/icecompany-tech/iceslab\n'
  printf '\n'
}
...
ICESLAB_NODE_REF=${ICESLAB_NODE_REF:-v0.1.8}
...
# Hysteria2 rides QUIC over UDP; on high-BDP paths (e.g. RU→EU) the default
# net.core.rmem_max (~208 KiB) caps quic-go's receive buffer and throttles
# throughput — users report the node "lagging". Lift the UDP socket buffers to
# 16 MiB and switch to BBR + fq (also helps the Reality/cascade TCP paths). The
# drop-in in /etc/sysctl.d survives reboots; default_qdisc only applies to NEW
# qdiscs, so also swap the live interface's qdisc to fq right now.
log "Applying QUIC/BBR network tuning (/etc/sysctl.d/99-kosatka-quic.conf)"
modprobe tcp_bbr 2>/dev/null || true
cat > /etc/sysctl.d/99-kosatka-quic.conf <<'SYSCTL'
# KOSATKA hy2/QUIC + BBR tuning — large UDP buffers for Hysteria2, BBR+fq for TCP.
...
SYSCTL
sysctl -p /etc/sysctl.d/99-kosatka-quic.conf >/dev/null 2>&1 || true
